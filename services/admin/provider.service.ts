...
export interface Provider {
  ...
  phone?: string | null;        // téléphone du prestataire
  city?: string;
  neighborhood?: string;
  street?: string;
  description?: string;
...
export const ProviderService = {
  ...
  /**
   * GET /admin/select/providers
   * Retourne la liste complète des prestataires actifs pour les sélecteurs.
   */
  async getSelectProviders(): Promise<Provider[]> {
    const response = await axios.get("/admin/select/providers");
    return response.data?.data ?? [];
  },
};
