...
import { Planning, InterventionReport, Asset } from "../../../../types/manager.types";
...
export default function ManagerPlanningDetailPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Fetch assets (site_id is handled by backend for manager, but we can pass it if multiple sites)
      const assetsData = await AssetService.getAssets({ per_page: 100 });
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h3 className="text-xl font-black text-slate-900 tracking-tight">Description du planning</h3>
              </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10">
              <div className="flex items-center justify-between mb-10">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-xl font-black text-slate-900 tracking-tight">Rapports global du planning</h3>
                </div>
              </div>
                  ...
                  {reports.map((report) => (
                    <div key={report.id} className="group flex flex-col md:flex-row md:items-center justify-between p-6 rounded-[1.8rem] border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-blue-100 hover:shadow-xl transition-all duration-300">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-1.5">
                            ...
                            <span className={`text-[9px] font-black px-3 py-1 rounded-full border uppercase tracking-widest ${report.status === "validated" ? "bg-emerald-50 border-emerald-100 text-emerald-600" : "bg-amber-50 border-amber-100 text-amber-600"
                              }`}>
                      ...
                      <div className="mt-6 md:mt-0">
                        <Link
                          href={`/manager/report-planning/${report.id}`}
              ...
              <div className="space-y-8">
                ...
                <div className="pt-8 border-t border-slate-50 space-y-4">
                  <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Wrench size={10} /> Équipements du site ({assets.length})
                  </h4>
                  <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                    {assets.length > 0 ? (
                      assets.map((asset) => (
                        <div key={asset.id} className="p-3 bg-white rounded-2xl border border-slate-100 flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400">
                            <Tag size={12} />
                          </div>
                          <div className="min-w-0">
                            <p className="text-[11px] font-black text-slate-900 truncate">{asset.designation}</p>
                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tight truncate">{asset.codification}</p>
                          </div>
                        </div>
                      ))
                    ) : (
                      <p className="text-[9px] text-slate-400 italic font-bold">Aucun équipement.</p>
                    )}
                  </div>
                </div>
              </div>
