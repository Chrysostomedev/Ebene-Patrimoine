// app/manager/report-planning/page.tsx
...
export default function ManagerReportPlanningPage() {
  ...
  // KPIs globaux gérés côté manager (filtré auto par le back sur ses sites)
  const [kpis, setKpis] = useState([
    { label: "Rapports globaux", value: "0", delta: "", trend: "up" as const },
    // { label: "Rapports de tickets", value: "0", delta: "", trend: "up" as const },
  ...
  const loadData = async (searchTerm: string) => {
    ...
    try {
      ...
      let res;
      if (activeTab === "global") {
        res = await ReportService.getReportsGlobal(params);
      } else {
        res = await ReportService.getReportsTicket(params);
      }
      ...
      // Récupération des statistiques filtrées par manager
      try {
        const stats = await ReportService.getStats({ intervention_type: "preventif" });
        setKpis([
          { label: "Rapports globaux", value: String(stats.total || 0), delta: "", trend: "up" as const },
          { label: "En attente", value: String(stats.pending || 0), delta: "", trend: "up" as const },
          { label: "Validés", value: String(stats.validated || 0), delta: "", trend: "up" as const },
        ]);
      } catch {
        /* Silencieux */
      }
  ...
  const columns = activeTab === "global" ? [
    ...
    },
    {
      ...
      render: (_: any, row: InterventionReport) => (
        <div className="flex items-center gap-2">
          <Link
            href={`/manager/report-planning/${row.id}`}
