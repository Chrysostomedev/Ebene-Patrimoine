// app/manager/report-planning/[id]/page.tsx
...
export default function ManagerReportPlanningDetailPage() {
  const { id } = useParams();
  ...
  const handleValidate = async () => {
    ...
    setSubmitting(true);
    try {
      // Le manager utilise validatePreventifReport ou validateReport
      await ReportService.validateReport(report.id, {
        result: validationResult,
        rating: validationRating,
        comment: validationComment || "Validé par le gestionnaire",
      });
          ...
          <div className="space-y-8">
            <div className="bg-white rounded-[32px] border border-slate-100 p-8 shadow-sm space-y-6">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <FileText size={14} />Actions ménées
              </h3>
                      ...
                      <div key={idx} className={containerClass}>
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            ...
                            {ticketForAsset && (
                              <Link
                                href={`/manager/preventif/${ticketForAsset.id}`}
