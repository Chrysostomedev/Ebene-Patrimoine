...
import {
  ...
  Download, Eye, Wrench, ArrowUpRight, CheckSquare, Info, PenSquare,
  MessageSquare, AlertCircle, ChevronLeft,
} from "lucide-react";
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setReport(data);
        const ticketId = (data as any).ticket_id || (data as any).ticket?.id;
        if (ticketId) {
          try {
            const [qs, invs] = await Promise.all([
              QuoteService.getQuotesByTicket(ticketId),
              InvoiceService.getInvoicesByTicket(ticketId),
            ]);
            setQuotes(qs);
            setInvoices(invs);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
                ...
                <div className="bg-slate-900 rounded-[32px] p-6 shadow-xl shadow-slate-200 space-y-4 border border-slate-800">
                  ...
                  <div className="space-y-2">
                    {allTickets.map((t: any, idx: number) => (
                      <Link
                        key={idx}
                        href={`/manager/preventif/${t.id || t.preventive_ticket_id}`}
            ...
            <div className="bg-white rounded-[32px] border border-slate-100 p-8 shadow-sm space-y-8">
              <div className="space-y-3">
                ...
                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100 italic text-sm text-slate-600 min-h-[100px] leading-relaxed">
              ...
              <div className="space-y-3">
                ...
                <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100 italic text-sm text-slate-600 min-h-[100px] leading-relaxed">
                  {report.action_taken || "Aucune action renseignée."}
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-slate-50 space-y-5">
                ...
                {/* Nouveau : Informations du Manager (Lui-même ou collègue) */}
                {report.site?.manager && (
                  <div className="flex items-center gap-3">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-[32px] border border-slate-100 p-8 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    {(() => {
                      const res = report.asset_results || (report as any).results || (report as any).assets || (report as any).items || [];
              ...
              <div className="space-y-4">
                {(() => {
                  let results = report.asset_results || (report as any).results || (report as any).assets || (report as any).items || [];
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center bg-slate-100/50 p-1 rounded-xl shrink-0 self-end md:self-auto">
                            ...
                            {res.preventive_ticket_id && (
                              <Link
                                href={`/manager/preventif/${res.preventive_ticket_id}`}
