// app/provider/report-planning/[id]/page.tsx
...
import {
  ...
  Wrench, Star, X, Info, ShieldAlert,
  AlertCircle, Eye
} from "lucide-react";
...
import { providerReportService, InterventionReport, getSiteName, getProviderName } from "../../../../services/provider/providerReportService";
...
export default function ProviderReportPlanningDetailPage() {
  const { id } = useParams();
  ...
  const loadReport = async () => {
    ...
    try {
      setLoading(true);
      const data = await providerReportService.getReportById(Number(id));
      setReport(data);
    } catch (err: any) {
  ...
  useEffect(() => {
    if (report?.site_id) {
      api.get(`/provider/asset/by-site/${report.site_id}`)
        .then(res => setSiteAssets(res.data?.data || res.data || []))
        .catch(console.error);
    }
  }, [report?.site_id]);
  ...
  const providerName = getProviderName(report.provider);
  const siteName = getSiteName(report.site);
                      ...
                      <div key={idx} className={containerClass}>
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            ...
                            {ticketForAsset && (
                              <Link
                                href={`/provider/preventif/${ticketForAsset.id}`}
