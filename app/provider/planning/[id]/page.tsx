...
export default function ProviderPlanningDetailPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setReports(Array.isArray(reportsData) ? reportsData : (reportsData as any)?.items || []);

      // Fetch assets if site_id is present
      if (planningData.site_id) {
        try {
          const assetsRes = await axiosInstance.get(`/provider/asset`, {
            params: { site_id: planningData.site_id, per_page: 100 }
          });
          setAssets(assetsRes.data?.data?.items || assetsRes.data?.data || []);
        } catch (e) {
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <main className="mt-24 p-8 flex flex-col items-center justify-center text-center">
          <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mb-6">
            <AlertCircle
              size={40} className="text-red-500" />
          </div>
      ...
      <main className="mt-28 p-10 lg:p-12 space-y-12 max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 relative overflow-hidden flex flex-col md:flex-row justify-between gap-10">
          ...
          <div className="flex flex-col gap-3 min-w-[240px]">
            {planning.status !== 'realise' && reports.length === 0 && (
              <Link
                href={`/provider/planning/${planning.id}/add`}
            ...
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <Shield size={14} /> Rapport global du planning
                </h3>
              </div>
              {reports.length > 0 ? (
                <div className="space-y-4">
                  {reports.map((report) => (
                    <div key={report.id} className="flex items-center justify-between p-5 rounded-2xl border border-slate-100 bg-slate-50/50">
                      ...
                      <Link
                        href={`/provider/report-planning/${report.id}`}
              ...
              <div className="pt-8 border-t border-slate-50 space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Wrench size={14} /> Patrimoines ({assets.length})
                  </h4>
                </div>
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {assets.length > 0 ? (
                    ...
                    <p className="text-[10px] text-slate-400 italic">Aucun équipement répertorié.</p>
                  )}
                </div>
              </div>
            </div>
