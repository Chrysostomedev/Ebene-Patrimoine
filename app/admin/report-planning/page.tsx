// app/admin/report-planning/page.tsx
...
export default function AdminReportPlanningPage() {
  ...
  const loadData = async (searchTerm: string) => {
    ...
    setError(null);
    try {
      // Détermination de la méthode API à appeler selon l'onglet actif
      const params = {
        page,
        per_page: 10,
      ...
      // Récupération des statistiques globales pour mettre à jour les KPIs dynamiques
      try {
        ...
        setKpis([
          ...
          // { label: "Rapports de tickets", value: String(stats.reports_by_type?.find(t => t.intervention_type === 'preventif')?.count || stats.total_reports || 0), delta: "", trend: "up" as const },
  ...
  const columns = activeTab === "global" ? [
    ...
    },
    {
      ...
      render: (_: any, row: InterventionReport) => (
        <div className="flex items-center gap-2">
          <Link
            href={`/admin/report-planning/${row.id}`}
